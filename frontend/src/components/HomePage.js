    ...
    const loadData = async () => {
      try {
        ...
        if (response.data && response.data.success) {
          ...
          // Cập nhật lại trạng thái like cho từng post
          const userId = user?.id || user?._id;
          const likedPosts = {};
          response.data.posts.forEach(post => {
            likedPosts[post._id] = post.likes.some(likeId =>
      ...
      try {
        ...
        if (response.data && response.data.success) {
          setPostComments(prev => ({
            ...prev,
            [postId]: response.data.comments // lấy từ backend
          }));
        }
        setCommentText(prev => ({
          ...prev,
                        ...
                        <div className="post-item">
                          ...
                          <div className="post-actions">
                            ...
                            >
                              ...
                              {userLikedPosts[post._id] ? 'Unlike' : 'Like'} {post.likes.length > 0 && `(${post.likes.length})`}
                            </button>
                            <button 
                              className="action-btn comment-btn"
                              onClick={() => handleToggleComment(post._id)}
                            >
                              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path>
                              </svg>
